...
(function () {
   ...
   // ============================================================
    ...
    // Níveis máximos por edifício - usado para detecção de edifícios completamente construídos
    var TW_BUILDING_MAX_LEVELS = {
        main:       30,
        barracks:   25,
        church:     15,
        watchtower: 20,
        ...
        snob:       1,
        smith:      20,
        place:      1,      // Praça só tem nível 1
        statue:     1,      // Estátua só tem nível 1 (nível 2 é upgrade da estátua existente)
        market:     25,
        wood:       30,
        stone:      30,
        iron:       30,
    ...
    function isButtonAvailable(buildingId, mainDoc) {
        // --- 0. Verificação de nível máximo primeiro ---
        var nivelAtual = parseInt((typeof game_data !== 'undefined' && game_data.village && game_data.village.buildings && game_data.village.buildings[buildingId]) ? game_data.village.buildings[buildingId].level : 0);
        var nivelMaximo = TW_BUILDING_MAX_LEVELS[buildingId] || 99;
        if (nivelAtual >= nivelMaximo) {
            return false; // Edifício já está no nível máximo
        }
        ...
        if (buildRow) {
            ...
            // Fallback: se a linha existe mas NÃO há nenhum botão/link de upgrade presente,
            // significa que o edifício está no máximo
            var upgradeBtn = buildRow.querySelector('.upgrade-button, .btn-upgrade, a[href*="ajaxaction"]');
     ...
     * @param {string} responseText - Resposta do servidor
     * @param {string} buildingId - ID do edifício construído
     * @param {string} villageId - ID da aldeia (opcional, para registro de edifícios maxed)
     */
    function verifyQueuedAfterBuild(responseText, buildingId, villageId) {
        try {
            ...
            if (json.error) {
                ...
                // Verificar se é erro de "edifício totalmente construído"
                var errorMsg = json.error.toLowerCase();
                if (errorMsg.indexOf('totalmente') !== -1 || 
                    errorMsg.indexOf('fully built') !== -1 || 
                    errorMsg.indexOf('maximum level') !== -1 ||
                    errorMsg.indexOf('max level') !== -1) {
                    // Registrar como edifício permanentemente maxed
                    if (villageId) {
                        registerMaxedBuilding(villageId, buildingId);
                    }
                }
                return false;
            }
            ...
            if (responseText.indexOf('error') !== -1 ||
                ...
                // Verificar mensagem de edifício totalmente construído no texto
                if (responseText.toLowerCase().indexOf('totalmente') !== -1 ||
                    responseText.toLowerCase().indexOf('fully built') !== -1 ||
                    responseText.toLowerCase().indexOf('maximum level') !== -1) {
                    if (villageId) {
                        registerMaxedBuilding(villageId, buildingId);
                    }
                }
    ...
    /**
     * Registra um edifício como permanentemente construído (nível máximo atingido)
     * @param {string} villageId - ID da aldeia
     * @param {string} buildingId - ID do edifício
     */
    function registerMaxedBuilding(villageId, buildingId) {
        try {
            var key = 'tw_maxed_buildings_' + villageId;
            var maxedRaw = GM_getValue(key, null);
            var maxedList = maxedRaw ? (typeof maxedRaw === 'string' ? JSON.parse(maxedRaw) : maxedRaw) : [];
            if (!maxedList.includes(buildingId)) {
                maxedList.push(buildingId);
                GM_setValue(key, JSON.stringify(maxedList));
                log('[maxed] Edifício ' + buildingId + ' registrado como NÍVEL MÁXIMO permanente em ' + villageId, 'success');
            }
        } catch (e) {
            log('[maxed] Erro ao registrar edifício maxed: ' + e.message, 'error');
        }
    }

    /**
     * Verifica se um edifício já foi registrado como nível máximo
     * @param {string} villageId - ID da aldeia
     * @param {string} buildingId - ID do edifício
     * @returns {boolean} true se o edifício está registrado como maxed
     */
    function isBuildingMaxed(villageId, buildingId) {
        try {
            var key = 'tw_maxed_buildings_' + villageId;
            var maxedRaw = GM_getValue(key, null);
            var maxedList = maxedRaw ? (typeof maxedRaw === 'string' ? JSON.parse(maxedRaw) : maxedRaw) : [];
            return maxedList.includes(buildingId);
        } catch (e) {
            return false;
        }
    }
    ...
    function isBuildExecutable(buildingId, state, mainDoc) {
        // 0. Verificar nível máximo primeiro (antes de qualquer outra verificação)
        var nivelAtual = parseInt(state.niveis[buildingId] || 0);
        var nivelMaximo = TW_BUILDING_MAX_LEVELS[buildingId] || 99;
        if (nivelAtual >= nivelMaximo) {
            return false; // Edifício já atingiu o nível máximo
        }

        // 0b. Verificar se está registrado como permanentemente maxed no GM storage
        if (state.villageId && isBuildingMaxed(state.villageId, buildingId)) {
            return false; // Edifício já foi registrado como nível máximo permanente
        }
...
// ============================================================
        ...
        if (state.filaBuilds < maxFila) {
            ...
            // Tudo agora compete no mesmo score com multiplicadores de urgência
            {
                ...
                var todosCandidatos = Object.keys(TW_BUILDING_REQS).filter(function(ed) {
                    ...
                    // Edifícios de utilidade restrita - só construir em fases específicas
                    if (ed === 'place') return false; // Nunca construir automaticamente (nível máximo 1)
                    if (ed === 'hide' && state.phase !== 'LATE') return false; // Esconderijo só late game
                    if (ed === 'statue' && (state.niveis['statue'] || 0) >= 1) return false; // Estátua já construída (max 1)
                    if (ed === 'church') return false; // Igreja só construção manual
                    if (ed === 'watchtower') return false; // Torre só construção manual
                    if (ed === 'garage') return false; // Oficina só construção manual
                    // Nobre: só quando puder produzir (HQ >= 20 e Smith >= 20)
                    if (ed === 'snob' && (parseInt(state.niveis['main'] || 0) < 20 || parseInt(state.niveis['smith'] || 0) < 20)) return false;
    ...
    async function bgBuildGeneric(villageId, building, csrf) {
        ...
        try {
            ...
            var success = verifyQueuedAfterBuild(resp, building, villageId);
            if (success) {
